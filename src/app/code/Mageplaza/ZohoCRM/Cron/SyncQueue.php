...
class SyncQueue
{
    ...
    public function execute()
    {
        ...
        if ($limitObject &&
            ...
            ($this->helperData->getDeveloperMode() === Mode::PRODUCTION)
        ) {
            $lastSchedule = (int) $this->helperData->getLastSchedule();
            $schedule     = (int) $this->helperData->getSchedule();
            $isValid      = ($lastSchedule + $schedule) < time();
            if ($isValid) {
                try {
                    $this->helperSync->setLimitObjectSend($limitObject);
                    $this->helperSync->syncs();
                    $this->helperData->saveLastSchedule();
                } catch (Exception $e) {
