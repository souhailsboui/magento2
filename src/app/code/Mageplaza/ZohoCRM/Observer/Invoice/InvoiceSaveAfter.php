...
class InvoiceSaveAfter extends AbstractQueue
{
    ...
    public function executeAction(Observer $observer)
    {
        ...
        if ($invoice->getZohoEntity() && !$invoice->hasQueueSave()) {
            $origData = $invoice->getCustomOrigData();
            $this->helperSync->updateObject($origData, $invoice, ZohoModule::INVOICE);
        } elseif (!$invoice->isObjectNew() && !$invoice->hasQueueSave()) {
            $this->helperSync->addObjectToQueue(ZohoModule::INVOICE, $invoice);
        }
    }
}
