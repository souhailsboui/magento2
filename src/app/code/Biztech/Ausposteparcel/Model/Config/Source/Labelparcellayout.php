...
class Labelparcellayout extends \Magento\Eav\Model\Entity\Attribute\Source\AbstractSource
{
    ...
    {
        if (!$this->_options) {
            $auspostLabel = $this->auspostlabelCollectionFactory->create()->addFieldToFilter('label_id', 1)->getFirstItem()->getData();

            if (sizeof($auspostLabel)) {
                $this->_options = array(
                    array('value' => '', 'label' => __('Select Label Layout for ' . $auspostLabel['label_group'])),
                    array('value' => 1, 'label' => __('A4-4pp')),
