...
class Article extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $returnAddressLine2 = trim($this->scopeconfiginterface->getValue('carriers/ausposteParcel/returnAddressLine2')!=null?$this->scopeconfiginterface->getValue('carriers/ausposteParcel/returnAddressLine2'):'');
        ...
        $returnAddressLine3 = trim($this->scopeconfiginterface->getValue('carriers/ausposteParcel/returnAddressLine3')!=null?$this->scopeconfiginterface->getValue('carriers/ausposteParcel/returnAddressLine3'):'');
        if (!empty($returnAddressLine3)) {
            $returnAddressLine3 = '<returnAddressLine3>' . trim($this->info->xmlData($returnAddressLine3)!=null?$this->info->xmlData($returnAddressLine3):'') . '</returnAddressLine3>';
        } else {
            $returnAddressLine3 = '<returnAddressLine3 />';
        }

        $returnAddressLine4 = trim($this->scopeconfiginterface->getValue('carriers/ausposteParcel/returnAddressLine4')!=null?$this->scopeconfiginterface->getValue('carriers/ausposteParcel/returnAddressLine4'):'');
        if (!empty($returnAddressLine4)) {
            $returnAddressLine4 = '<returnAddressLine4>' . trim($this->info->xmlData($returnAddressLine4)!=null?$this->info->xmlData($returnAddressLine4):'') . '</returnAddressLine4>';
        } else {
    ...
    public function prepareArticles($data, $order)
    {
        ...
        for ($i = $start_index; $i <= $end_index; $i++) {
            if ($data['articles_type'] == 'Custom') {
                ...
                $article['description'] = trim($articles[1]);
                $article['weight'] = $articles[2];
                $article['height'] = $articles[3];
                $article['width'] = $articles[4];
                $article['length'] = $articles[5];
    ...
    public function prepareModifiedArticleData($order, $consignment_number = '')
    {
        ...
        $chargeCode = $this->_objectManager->create('Biztech\Ausposteparcel\Helper\Info')->getChargeCode($order, $consignment_number);
        $instructions = ''; //Mage::helper('ausposteParcel/info')->getInstructions($order);
