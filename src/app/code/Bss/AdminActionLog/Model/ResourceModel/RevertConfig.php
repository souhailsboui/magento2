...
{
    /**
     * @var \Bss\AdminActionLog\Model\ActionDetail
     */
    ...
    public function revertConfig($log_id)
    {
        ...
        if ($collecttion->getSize() > 0) {
            foreach ($collecttion as $logDetail) {
                $new_value = json_decode($logDetail->getNewValue(), true);
                $old_values = json_decode($logDetail->getOldValue(), true);
