...
class AjaxMassWidthMd extends AbstractAjaxFieldsetMassAction
{
    const ACTION = 'update';
    ...
    {
        $newValue = $this->getRequest()->getParam(FieldsetInterface::WIDTH_PROPORTION_MD);
        foreach ($collection as $item) {
            $model = $this->repository->getById($item->getId());
            $model->setWidthProportionMd($newValue);
